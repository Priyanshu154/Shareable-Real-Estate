...
def buy_share(request, prop_id):
    ...
    if request.method == "POST":
        ...
        amount = prop.price_per_share * no_of_shares

        res=  make_transaction( debitID= person.account_id, creditID=settings.ADMIN_ACCOUNT_ID , amount= amount)
        if res == -1:
            return redirect('payment_failure',"Payment Error, Insufficient Funds !")
        else:
            transaction_id = res['transferID']
        print(prop_id)
        buyer_data = {'prop_id': prop_id, 'buy_share': no_of_shares, 'money': amount}
...
def payment_success(request, buyer_id):
    ...
    amount = buyer.buyer_shares*buyer.buyer_property.price_per_share
    return render( request, 'payment_success.html', {'buyer': buyer, 'prop': buyer.buyer_property, 'amount': amount } )

def payment_failure(request,msg):
    return render(request, 'payment_failure.html',{'msg':msg})
